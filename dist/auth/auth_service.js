...
class AuthServices {
    ...
    static async create(name, id, role, dob, email, phonenumber) {
        ...
        const insert_res = await db_1.db.query(query, values);
    ...
    static async login(id, pass) {
        ...
        const data = await db_1.db.query(query, [id]);
        if (data.rows.length == 0)
            throw new Error('User Not Found ');
        const hashed_pass = data.rows[0]?.password;
        console.log('hashed_pass', hashed_pass, "password", pass);
        const verified = await pass_1.PasswordService.verify(hashed_pass, pass);
        console.log('verified..', verified);
        if (!verified)
            throw new Error('Unauthorized credentials');
        const needrehash = pass_1.PasswordService.needsRehash(hashed_pass);
        if (needrehash) {
            const newpass = await pass_1.PasswordService.hash(pass);
            const update_query = `UPDATE  app.profile SET password=$1 WHERE id_number=$2`;
            const update_values = [newpass, id];
            const update = await db_1.db.query(update_query, update_values);
        }
        const user = data.rows[0];
        const token = jwt_1.TokenService.generate(data.rows[0]?._id);
        console.log('generated token...', token);
        const create_token = await db_1.db.query(`INSERT INTO app.active_tokens(userid,token,expires_at,created_at) VALUES ($1,$2,NOW()+INTERVAL'30 minutes',NOW())`, [data.rows[0]._id, token]);
        if ((create_token.rowCount ?? 0) < 1)
            throw new Error('error in inserting token to db');
        const { password: _, ...safeUser } = user;
        return { ...safeUser, token };
    }
    static async updatePass(newpass, pass, id) {
        const validate_query = `SELECT password FROM app.profile WHERE id_number=$1`;
        const v_results = await db_1.db.query(validate_query, [id]);
        const validate_user = await pass_1.PasswordService.verify(v_results.rows[0].password, pass);
        if (!validate_user)
            throw new Error("user not verified");
        console.log('updatingg....');
        const update_query = `UPDATE app.profile SET password=$1 WHERE id_number=$2`;
        const hashednew = await pass_1.PasswordService.hash(newpass);
        const values = [hashednew, id];
        const results = await db_1.db.query(update_query, values);
    ...
    static async logout(id) {
        const loggingout = await db_1.db.query(`DELETE * FROM app.active_token WHERE userid=$1`, [id]);
...
class AuthController {
    ...
    async getuser(req, res) {
        try {
            ...
            const { rows } = await db_1.db.query(query, [data.userId]);
