...
exports.getBillClauses = getBillClauses;
exports.getActiveSession = getActiveSession;
exports.getBillCommitteeStatus = getBillCommitteeStatus;
exports.openCommitteeSession = openCommitteeSession;
exports.castCommitteeVote = castCommitteeVote;
exports.closeCommitteeSession = closeCommitteeSession;
exports.getSenatorCommitteeVote = getSenatorCommitteeVote;
// import { pool } from '../db';
...
async function getBillClauses(billId) {
    const clauseRes = await db_1.db.query(`SELECT * FROM app.clauses WHERE bill_id = $1 ORDER BY order_index ASC`, [billId]);
    const clauses = clauseRes.rows;
    // Fetch amendments for each clause and attach votes from closed sessions
    for (const clause of clauses) {
        const amdRes = await db_1.db.query(`SELECT a.*,
...
async function getActiveSession(billId) {
    const res = await db_1.db.query(`SELECT * FROM app.committee_voting_sessions
     WHERE bill_id = $1 AND status = 'open'
     LIMIT 1`, [billId]);
    return res.rows[0] ?? null;
}
// ─── READ: Bill committee status summary ─────────────────────────────────────
async function getBillCommitteeStatus(billId) {
    const [clauseStats, amdStats, active] = await Promise.all([
       ...
       FROM app.clauses WHERE bill_id = $1`, [billId]),
        db_1.db.query(`SELECT
         COUNT(*) as total,
         COUNT(*) FILTER (WHERE status != 'pending') as voted
       FROM app.amendments WHERE bill = $1`, [billId]),
...
async function openCommitteeSession(params) {
    ...
    try {
        ...
        const existing = await client.query(`SELECT id FROM app.committee_voting_sessions WHERE bill_id = $1 AND status = 'open'`, [billId]);
        ...
        if (itemType === 'amendment' && amendmentId) {
            const amd = await client.query(`SELECT * FROM app.amendments WHERE _id = $1`, [amendmentId]);
            if (!amd.rows[0])
        ...
        if (itemType === 'clause' && clauseId) {
            const unvoted = await client.query(`SELECT _id FROM app.amendments WHERE clause_id = $1 AND status = 'pending'`, [clauseId]);
            if (unvoted.rows.length > 0) {
                throw new Error('All amendments must be voted on before voting on the clause.');
            }
        }
        const res = await client.query(`INSERT INTO app.committee_voting_sessions
       ...
       RETURNING *`, [
        ...
        let label = '';
        if (itemType === 'amendment' && amendmentId) {
            const amd = await db_1.db.query(`SELECT preview FROM app.amendments WHERE _id = $1`, [amendmentId]);
            label = `Amendment: ${amd.rows[0]?.preview ?? ''}`;
        }
        else if (itemType === 'clause' && clauseId) {
            const cl = await db_1.db.query(`SELECT number, title FROM app.clauses WHERE id = $1`, [clauseId]);
            label = `Clause ${cl.rows[0]?.number}: ${cl.rows[0]?.title ?? ''}`;
        }
        else if (itemType === 'en_bloc' && enBlocClauseIds?.length) {
            const cls = await db_1.db.query(`SELECT number FROM app.clauses WHERE id = ANY($1) ORDER BY order_index`, [enBlocClauseIds]);
        ...
        (0, socket_1.broadcastCommittee)(billId, {
...
// ─── SENATOR: Cast a vote ─────────────────────────────────────────────────────
async function castCommitteeVote(sessionId, senatorId, choice) {
    const client = await db_1.db.connect();
    try {
        await client.query('BEGIN');
        const sessionRes = await client.query(`SELECT * FROM app.committee_voting_sessions WHERE id = $1 FOR UPDATE`, [sessionId]);
        ...
        // Prevent double vote
        const already = await client.query(`SELECT id FROM app.committee_votes WHERE session_id = $1 AND senator_id = $2`, [sessionId, senatorId]);
        if (already.rows.length > 0)
            throw new Error('You have already voted in this session');
        // Record vote
        const voteRes = await client.query(`INSERT INTO app.committee_votes
         (session_id, senator_id, bill_id, item_type, choice, voted_at)
       VALUES ($1, $2, $3, $4, $5, NOW())
       RETURNING *`, [sessionId, senatorId, session.bill_id, session.item_type, choice]);
        // Increment tally
        const col = choice === 'accept' ? 'total_accept' : choice === 'reject' ? 'total_reject' : 'total_abstain';
        const updated = await client.query(`UPDATE app.committee_voting_sessions
       SET ${col} = ${col} + 1
       WHERE id = $1
       RETURNING total_accept, total_reject, total_abstain`, [sessionId]);
        ...
        (0, socket_1.broadcastCommittee)(session.bill_id, {
...
async function closeCommitteeSession(sessionId, speakerId, stage) {
    const stageactions = new action_1.StageActions();
    const client = await db_1.db.connect();
    try {
        await client.query('BEGIN');
        const sessionRes = await client.query(`SELECT * FROM app.committee_voting_sessions WHERE id = $1 AND opened_by = $2 FOR UPDATE`, [sessionId, speakerId]);
        if (!sessionRes.rows[0])
        ...
        const affectedClauseIds = [];
        // Close the session
        const closed = await client.query(`UPDATE app.committee_voting_sessions SET status='closed', closed_at=NOW(), passed=$1 WHERE id=$2 RETURNING *`, [passed, sessionId]);
        // ── Update the relevant item's status based on result ──────────────────
        if (session.item_type === 'amendment' && session.amendment_id) {
            await client.query(`UPDATE app.amendments SET status = $1 WHERE _id = $2`, [passed ? 'passed' : 'failed', session.amendment_id]);
            ...
            if (amd.rows[0])
                affectedClauseIds.push(amd.rows[0].clause_id);
        }
        if (session.item_type === 'clause' && session.clause_id) {
            await client.query(`UPDATE app.clauses SET status = $1 WHERE id = $2`, [passed ? 'passed' : 'failed', session.clause_id]);
        ...
        if (session.item_type === 'en_bloc' && session.en_bloc_clause_ids?.length) {
            ...
            await client.query(`UPDATE app.clauses SET status = $1 WHERE id = ANY($2)`, [passed ? 'passed' : 'failed', session.en_bloc_clause_ids]);
            affectedClauseIds.push(...session.en_bloc_clause_ids);
        }
        // ── Check if bill should be killed ────────────────────────────────────
        const clauseStats = await client.query(`SELECT
         COUNT(*) as total,
         COUNT(*) FILTER (WHERE status = 'failed') as failed,
         COUNT(*) FILTER (WHERE status = 'passed') as passed,
         COUNT(*) FILTER (WHERE status = 'abstain') as abstain
       FROM app.clauses WHERE bill_id = $1`, [session.bill_id]);
...
async function getSenatorCommitteeVote(sessionId, senatorId) {
    const res = await db_1.db.query(`SELECT * FROM app.committee_votes WHERE session_id = $1 AND senator_id = $2`, [sessionId, senatorId]);
    return res.rows[0] ?? null;
}
