...
const committeestageService_1 = require("../services/committeestageService");
...
router.post('/clauses', asyncHandler(async (req, res) => {
    ...
    const clauses = await (0, committeestageService_1.getBillClauses)(billId);
    res.json({ clauses });
}));
...
router.post('/status', asyncHandler(async (req, res) => {
    ...
    const status = await (0, committeestageService_1.getBillCommitteeStatus)(billId);
    res.json(status);
}));
// ── GET /api/committee/:billId/active-session ─────────────────────────────────
router.get('/:billId/active-session', asyncHandler(async (req, res) => {
    const session = await (0, committeestageService_1.getActiveSession)(req.params.billId);
...
router.post('/sessions/open', asyncHandler(async (req, res) => {
    ...
    const session = await (0, committeestageService_1.openCommitteeSession)({
        billId: bill_id,
...
router.post('/sessions/:sessionId/vote', asyncHandler(async (req, res) => {
    ...
    const result = await (0, committeestageService_1.castCommitteeVote)(sessionId, senator_id, choice);
...
router.post('/sessions/:sessionId/close', asyncHandler(async (req, res) => {
    ...
    const result = await (0, committeestageService_1.closeCommitteeSession)(sessionId, speaker_id, stage);
...
router.get('/sessions/:sessionId/my-vote', asyncHandler(async (req, res) => {
    ...
    const vote = await (0, committeestageService_1.getSenatorCommitteeVote)(sessionId, senator_id);
