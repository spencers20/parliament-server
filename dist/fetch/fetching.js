...
LEFT JOIN app.stages dead_s   ON dead_s._id = db.stage
             `);
            if (rows.length === 0)
                res.status(200).json({ message: 'No bills available' });
    ...
    static async specificbill(billId) {
        try {
            ...
            if (rows.length === 0)
                throw new Error('Error in getting the stages');
    ...
    static async committee(req, res) {
        try {
            const { rows } = await db_1.db.query(`
    ...
    static async specificbillstage(billId) {
        try {
            const { rows } = await db_1.db.query(`
    ...
    //     }
    // }
    static async specificcommittee(comId) {
        try {
            const { rows } = await db_1.db.query(`
    ...
    static async committeereport(stageid) {
        try {
            const { rows } = await db_1.db.query(`
    ...
    static async specificstage(stageId) {
        try {
            const { rows } = await db_1.db.query(`
    ...
    static async committeebills(req, res) {
        try {
